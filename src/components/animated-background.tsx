...
// Load Spline as a fully client-side async chunk with no SSR.
// Keeping this dynamic import here (not at module top) ensures Turbopack
// never attempts to resolve @splinetool/react-spline during SSR.
...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  // Guard: delay rendering <Spline> until the next event-loop tick so the
  // Emscripten WASM binary has time to call assignWasmExports() and expose
  // _malloc before SubdivGeometry tries to allocate memory.
  const [wasmReady, setWasmReady] = useState(false);
  useEffect(() => {
    const id = setTimeout(() => setWasmReady(true), 0);
    ...
    <Suspense fallback={<div>Loading...</div>}>
      {wasmReady && (
        <Spline
          ...
          scene="https://prod.spline.design/QVRY2mCDcs5diEsH/scene.splinecode"
        />
      )}
    </Suspense>
  );
