...
import { fetchProjects, convertApiProject } from "@/lib/project-adapter";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";

type Project = ReturnType<typeof convertApiProject>;
  ...
  const loadProjects = async () => {
    try {
      ...
      if (Array.isArray(apiProjects)) {
        setProjects(apiProjects.map(convertApiProject));
      } else {
        setProjects([]);
      }
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center justify-center">
              <div className="w-full max-w-[400px] h-auto rounded-lg overflow-hidden ring-1 ring-white/5 animate-pulse" style={{ aspectRatio: "3/2" }}>
      ...
      <ResponsiveDialog>
        <ResponsiveDialogTrigger className="bg-transparent flex justify-center w-full">
          ...
          >
            <ScrollingPreview
              src={project.src}
              alt={project.title}
              bg={`/assets/backgrounds/${project.id}.svg`}
            />
        ...
        <ResponsiveDialogContent className="md:max-w-4xl md:h-[85vh] md:!flex md:flex-col md:overflow-hidden md:p-0 md:gap-0">
          <ResponsiveDialogDescription className="sr-only">
            {project.title} details and project overview
          </ResponsiveDialogDescription>
              ...
              >
                <div className="prose prose-invert max-w-none">
                  ...
                  {(!project.content || project.content.trim() === "") && (
                    <p className="font-mono text-center text-muted-foreground">
                  ...
                  {(project.github && project.github !== "#") || (project.live && project.live !== "#") ? (
                    <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-8 border-t border-border">
                      {project.github && project.github !== "#" && (
                        <Link href={project.github} target="_blank" className="font-mono underline flex gap-2 inline-block">
                          <button className="group flex items-center gap-2 bg-primary text-primary-foreground text-sm font-medium px-4 py-1.5 rounded-full hover:bg-primary/80 transition-colors">
                            View Source
                            <ArrowUpRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                          </button>
